...
% load data for 장애물 체크
load('MapDATA.mat','unfreeMap','resolution','unfreeSpaceID');
...
for i = 1:1:numNodes
    ...
    if noCollision2(q_rand, q_near.coord, unfreeMap,resolution,unfreeSpaceID)
        ...
        neighbor_count = 1;
        for j = 1:1:length(nodes)
            if noCollision2(nodes(j).coord, q_new.coord, unfreeMap,resolution,unfreeSpaceID) && dist(nodes(j).coord, q_new.coord) <= r
        ...
        for k = 1:1:length(q_nearest)
            if noCollision2(q_nearest(k).coord, q_new.coord, unfreeMap,resolution,unfreeSpaceID) && q_nearest(k).cost + dist(q_nearest(k).coord, q_new.coord) < C_min
                q_min = q_nearest(k);
