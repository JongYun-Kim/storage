...
%% Get Gnr matrix
    relay_position = reshape(p(particle,:),2,NumRelay)';  % 각 행에 각 릴레이의 (x,y) position있음
    ...
    map_out_num = sum(unfreeMap(relay_position_gridID_short));  % 몇 대의 릴레이가 맵 밖으로 나가있는지
    if map_out_num  % 만약에 맵 밖에 한 대 이상 있다면.
        if GMC
            val(particle,1) = 100*(NumNodes+NumRelay);       % 로봇 갯수 만큼 최대 val으로 집어 넣기
            tree(particle).Gnr2 = zeros(NumNodes+NumRelay);  % dummy tree 만듬 : 나중에 이거 맵 밖으로 나간 릴레이 있다고 알아서 생각하셈
        elseif mGMC
    ...
    Gnr_ground_to_relay = -1*ones(NumNodes,NumRelay);    % GPR데이터로 구할 수 있는 부분 (Gnr의 오른쪽 부분(아래 빼고))
...
% % %     end
    ...
    Gnr_bw_relay = -1*ones(NumRelay);   % Gnr의 우하단 정사각 행렬; 릴레이 간의 통신 adj 행렬 (음수행렬)
