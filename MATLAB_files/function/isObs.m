function existence = isObs(x1,y1,x2,y2,unfreeMap,resolution,unfreeSpaceID)
% 두 점을 넣었을 때, 입력 받은 그리드 맵에서 두 가지 거리를 계산함
% d1 : free space를 통과하는 거리
% d2 : obstacle을 통과하는 거리
% 실제 두 점간 거리는 d = d1 + d2
% (x1,y1), (x2,y2) : 두 점 R1-R2
...
%              x(k)       x(k+1)
    ...
    x_its_wrt_y = m^-1*(y_range-y1) + x1;   % y(j)
...
%     seg_if_unfree = ismember(seg_id1, unfreeSpaceID); % free이면 0  unfree이면 1
    if sum(ismember(seg_id1, unfreeSpaceID))
        existence = 1; % obs 있는 경우
    else
        existence = 0; % obs 없는 경우
    end
