...
pruning = true;    % 짧은 거리로 감(true)// 이거 사실 pruning이 아니고 waypoint reduction임...
    if pruning
        if sc_fire
            pathtype = 1;  % 직선경로
        elseif sc_many
            pathtype = 4;  % 임시입니다ㅠㅠㅠ 이거 해제 해야할 수도 있습니다.
        end
    else
        pathtype = 2;  % 꼬불꼬불 경로
    end
...
for trial = 1:NumTrial
    DATA_Gao(trial).WCC = [];
    DATA_Gao(trial).relay = [];
    DATA_Gao(trial).signals = [];
    DATA_Gao(trial).path = [];
end
...
% 저장된 것을 사용할 경우
% load('./data/path_data/tempPath_sn01_r3.mat');
% load('./data/path_data/AllPath.mat','Mission');
load('./data/path_data/AllPath.mat','Mission2');  Mission = Mission2; clear Mission2;


%% 몬테 카를로!
for trial = 1:NumTrial
    path = Mission(mission).pathtype(pathtype).cases(trial).data;  % 경로는 미션과 경로타입별로 잘 선택하자
    ...
    disp(best_val);
    % 결과 저장
    DATA_Gao(trial).WCC = best_val;
    for rr = 1:NumRelay
        DATA_Gao(trial).relay(rr,:) = path(best_idx(rr),:);
    end
    DATA_Gao(trial).signals = best_Sall;
    DATA_Gao(trial).path = path;
...
for trial = 1:NumTrial
    for i = 1:NumRelay
        rx = DATA_Gao(trial).relay(i,1);
        ry = DATA_Gao(trial).relay(i,2);
