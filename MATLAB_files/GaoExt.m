...
% % % %     load('tempPath_ext.mat');
% % % %     result_manyGao(i).path = path;
end
...
%disp(best_idx);
%disp(best_val);
    result_GaoExt(trial).val_hist = val_hist;
    result_GaoExt(trial).relay_idx_hist = relay_idx_hist;
    result_GaoExt(trial).best_val = best_val;
    result_GaoExt(trial).best_idx = best_idx;
    result_GaoExt(trial).best_Sall = best_Sall;
    result_GaoExt(trial).best_relay = path(best_idx,:);
...
hold on; grid off;
legend_str = cell(((NumRelay*NumPath)+NumBase+NumClient+1+NumPath),1);
legend_str{1} = 'Map';

% path plot
for i = 1:NumPath
    plot3(result_GaoExt(i).path(:,1),result_GaoExt(i).path(:,2),ones(size(result_GaoExt(i).path,1),1),'LineWidth',1.5);
...
for trial = 1:NumPath
    for i = 1:NumRelay
        plot_relay(trial,i) = plot3(result_GaoExt(trial).best_relay(i,1),result_GaoExt(trial).best_relay(i,2),1,'p','MarkerSize',6,'LineWidth',4);
        legend_str{1+NumPath+(i+(NumRelay*(trial-1)))} = ['Relay',num2str(trial),num2str(i)];
    end
end
...
for i = 1:NumBase
    ...
    legend_str{1+NumPath+(NumRelay*NumPath)+i} = ['Base',num2str(i)];
end
...
%     plot3(initial_position_client(i,1),initial_position_client(i,2),1,'v','Color','b','MarkerSize',4,'LineWidth',4)
    ...
    legend_str{1+NumPath+(NumRelay*NumPath)+NumBase+i} = ['Client',num2str(i)];
end
