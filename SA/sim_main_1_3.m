function [assignment, damage_tot, damage_each, dam_tot_unc, dam_each_unc] = sim_main_1_3(r,pk,v_s,phi)      %uav_list,ship_list,n_u,n_s,sigma)

%% Information Settings
    rng('shuffle') % random seed generator
    
    % Ship infomation
    ship_speed = v_s; % 15 ;                    % speed of ships[m/s]
    ship_heading = phi;                          % heading angle of the swarm of ships ranged by [-pi +pi]
    ship_position = [ 5000, 0 ] ; % 2 column vectrors [Xs,Ys] % position of the ships when they meet uavs [m]
    n_s = size(ship_position,1) ;     % the number of ships (targets)
    ship_attack_radi = repmat(r,1,n_s);        % row vector % radius of attack zones of ships [m]
    ship_energy = repmat(8,1,n_s);               % relative energies of ships
    K_P = repmat(pk,1,n_s);                        % row vector  % killing ability of a ship [uav/s]
%     K_P = ship_spd_model(ship_speed,K_P);
    
    % UAV infomation
    uav_position = zeros(8,2); % 2 column vectors [Xs,Ys]
